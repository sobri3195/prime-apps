...
export function LaporanPage() {
  ...
  return <section className="space-y-6 pb-24">{/* shortened for brevity */}
    ...
    <section className="rounded-3xl bg-white p-4"><h2>Hasil AI Mata Terakhir</h2><p className="text-sm">Gejala mata buram dan merah terdeteksi. Disarankan melakukan pemeriksaan lanjutan ke dokter mata.</p><button onClick={()=>openModal('aiDetail')} className="prime-interactive prime-cta-dark mt-2 rounded-xl px-3 py-2">Lihat Detail AI</button></section>
    <section className="rounded-3xl bg-white p-4"><h2>Rekomendasi</h2><p className="text-sm">{recommendation}</p><button onClick={()=>{setBookingData((v)=>({...v,service:'Pemeriksaan Mata Lengkap'}));openModal('booking');setToastMessage('Silakan pilih jadwal pemeriksaan.');}} className="prime-interactive prime-cta-gold mt-3 rounded-xl px-4 py-2 text-white">Booking Pemeriksaan</button></section>
