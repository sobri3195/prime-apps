...
export function LaporanPage() {
  ...
  return <section className="space-y-6 pb-24">{/* shortened for brevity */}
    ...
    <section className="rounded-3xl bg-white p-4"><h2>Hasil AI Mata Terakhir</h2><p className="text-sm">Gejala mata buram dan merah terdeteksi. Disarankan melakukan pemeriksaan lanjutan ke dokter mata.</p><button onClick={()=>openModal('aiDetail')} className="prime-interactive mt-2 rounded-xl bg-prime-cream/50 px-3 py-2">Lihat Detail AI</button></section>
    <section className="rounded-3xl bg-white p-4"><h2>Rekomendasi</h2><p className="text-sm">{recommendation}</p><button onClick={()=>{setBookingData((v)=>({...v,service:'Pemeriksaan Mata Lengkap'}));openModal('booking');setToastMessage('Silakan pilih jadwal pemeriksaan.');}} className="prime-interactive prime-gold-button mt-3 rounded-xl px-4 py-2 text-white">Booking Pemeriksaan</button></section>
    ...
    {selectedModal && <div className="fixed inset-0 z-50 flex items-end justify-center bg-black/40 p-4" onClick={closeModal}><div className="max-h-[85vh] w-full max-w-[410px] overflow-auto rounded-3xl bg-white p-4" onClick={(e)=>e.stopPropagation()}><div className="mb-3 flex justify-between"><h3 className="font-semibold">Detail</h3><button onClick={closeModal}><X size={18}/></button></div>
      ...
      ].map(([label,fn,Icon]:any)=><button key={label} onClick={fn} className="prime-interactive flex w-full items-center gap-2 rounded-xl bg-prime-cream/40 p-3"><Icon size={16}/>{label}</button>)}</div>}
      {selectedModal==='metric' && <div><p className="font-semibold">{selectedDetail?.title}</p><p className="mt-2 text-sm">{selectedDetail?.detail}</p><div className="mt-3 flex gap-2"><button onClick={()=>openModal('historyDetail',examinationHistory[0])} className="prime-interactive rounded-xl bg-prime-cream/50 px-3 py-2">Lihat Riwayat</button><button onClick={()=>openModal('booking')} className="prime-interactive prime-gold-button rounded-xl px-3 py-2 text-white">Booking Pemeriksaan</button></div></div>}
      ...
      {selectedModal==='historyDetail' && <div><p className="font-semibold">{selectedDetail?.title}</p><p className="text-sm">{selectedDetail?.result}</p><div className="mt-3 flex gap-2"><button onClick={downloadSummaryReport} className="prime-interactive rounded-xl bg-prime-cream/50 px-3 py-2">Unduh Hasil</button><button onClick={()=>openModal('booking')} className="prime-interactive prime-gold-button rounded-xl px-3 py-2 text-white">Booking Kontrol</button></div></div>}
      {selectedModal==='aiDetail' && <div className="space-y-1 text-sm"><p>Tanggal screening: {latestAIResult.date}</p><p>Keluhan: {latestAIResult.complaint}</p><p>Tingkat risiko: {latestAIResult.riskLevel} ({latestAIResult.riskScore})</p><p>{latestAIResult.recommendation}</p><div className="mt-2 flex gap-2"><button onClick={()=>openModal('booking')} className="prime-interactive prime-gold-button rounded-xl px-3 py-2 text-white">Booking Dokter Mata</button><button onClick={()=>setToastMessage('Screening AI berhasil diulang (simulasi).')} className="prime-interactive rounded-xl bg-prime-cream/50 px-3 py-2">Ulangi Screening AI</button></div></div>}
