...
const CHECKED_IN_MESSAGE =
  'Anda sudah check-in hari ini. Kembali lagi besok untuk menjaga streak Anda.';
const INSUFFICIENT_POINTS_MESSAGE =
  'Poin belum cukup untuk menukar reward ini.';
...
const createInitialMissions = (): DailyMission[] => [
  ...
  },
  {
    ...
    description:
      'Screening awal membantu Anda tahu langkah lanjutan yang tepat.',
    ...
    (set, get) => {
      ...
      const normalizeMissedStreak = () => {
        ...
        if (
          ...
          dateDiffInDays(user.lastCheckInDate) > 1 &&
      ...
      return {
        ...
        badges: [
          {
            ...
            description:
              'Badge pertama untuk pasien yang berhasil melakukan check-in pertama.',
            icon: 'medal',
          ...
          },
          {
            ...
            name: '7 Day Eye Care',
            description:
              'Jaga streak check-in kesehatan mata selama 7 hari berturut-turut.',
            icon: 'flame',
        ...
        rewards: [
          {
            ...
            description:
              'Potongan biaya untuk pemeriksaan mata berikutnya di Klinik Utama Prime.',
            pointsRequired: 150,
            rewardType: 'voucher',
            value: 'Rp 10.000',
            isAvailable: true,
          },
          {
            ...
            description:
              'Diskon untuk pembelian vitamin mata, tetes mata, atau aksesori optik.',
            pointsRequired: 180,
            rewardType: 'voucher',
            value: '5%',
            isAvailable: true,
          },
          {
            ...
            description:
              'Sesi konsultasi singkat untuk pertanyaan ringan seputar kesehatan mata.',
            pointsRequired: 250,
          ...
          },
          {
            ...
            description:
              'Prioritas antrean satu kali saat kunjungan pemeriksaan.',
            pointsRequired: 300,
        ...
        checkInDaily: (userId) => {
          ...
          set((current) => ({
            ...
            dailyMissions: current.dailyMissions.map((mission) =>
              mission.missionType === 'daily_check_in'
                ? { ...mission, isCompleted: true, completedAt: now }
                : mission,
            ),
            error: null,
          }));
          ...
          get().addPoints(
            userId,
            'daily_check_in',
            POINT_RULES.daily_check_in,
            activityDescriptions.daily_check_in,
          );
          get().unlockBadgeIfEligible(userId);

          return {
            success: true,
            message: 'Check-in berhasil! Streak Anda aman hari ini ✨',
          };
        },
        ...
        addPoints: (userId, activityType, points, description) => {
          ...
          set((state) => ({
            user: {
              ...
              level:
                Math.floor((state.user.totalPoints + pointValue) / 100) + 1,
              updatedAt: getNow(),
            },
            counts: {
              ...state.counts,
        ...
        completeMission: (userId, missionId) => {
          ...
          const mission = get().dailyMissions.find(
          ...
          return {
            success: true,
            message: `Misi selesai. +${mission.pointReward} poin ditambahkan.`,
          };
        },
        ...
        unlockBadgeIfEligible: () => {
          ...
          set({
            badges: state.badges.map((badge) => {
              ...
              const shouldUnlock =
                (badge.id === 'mata-sehat-starter' &&
                  state.counts.daily_check_in >= badge.requirementValue) ||
                (badge.id === 'seven-day-eye-care' &&
                  state.user.currentStreak >= badge.requirementValue) ||
                (badge.id === 'ai-screening-hero' &&
                  state.counts.ai_screening_completed >=
                    badge.requirementValue) ||
                (badge.id === 'rajin-kontrol' &&
                  state.counts.booking_created >= badge.requirementValue) ||
                (badge.id === 'prime-care-member' &&
                  state.user.totalPoints >= badge.requirementValue);

              return shouldUnlock
                ? { ...badge, isUnlocked: true, unlockedAt: now }
                : badge;
            }),
          });
        },
        ...
        redeemReward: (userId, rewardId) => {
          ...
          const reward = state.rewards.find(
            (item) => item.id === rewardId && item.isAvailable,
          );
          ...
          set((current) => ({
            user: {
              ...
              level:
                Math.floor(
                  (current.user.totalPoints - reward.pointsRequired) / 100,
                ) + 1,
              updatedAt: getNow(),
            },
            counts: {
          ...
          return {
            success: true,
            message: `${reward.title} berhasil ditukar.`,
          };
        },
        ...
        getAvailableRewards: () =>
          get().rewards.filter((reward) => reward.isAvailable),
      };
    },
    {
      name: 'prime-apps-gamification',
      partialize: (state) => ({
        user: state.user,
        badges: state.badges,
